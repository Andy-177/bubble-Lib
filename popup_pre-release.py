...
class BubbleBox:
    ...
    def __init__(self, title="消息提示", text="这是一条消息", color="#800080", height=50):
        self.root = tk.Toplevel()  # 使用 Toplevel 创建独立窗口
        self.title = title
        self.text = text
    ...
    def _start_next_instance(cls):
        ...
        if not cls.task_queue.empty():
            title, text, color, height = cls.task_queue.get()  # 获取下一个任务的参数
            cls.current_instance = BubbleBox(title, text, color, height)  # 实例化并显示下一个气泡框
        else:
            cls.current_instance = None  # 没有更多实例时，清除当前实例

    @classmethod
    def enqueue_bubble(cls, title, text, color, height=50):
        """将创建气泡框的任务参数放入队列中"""
        cls.task_queue.put((title, text, color, height))  # 存储气泡框的参数
...
if __name__ == "__main__":
    ...
    # 在主线程中创建多个气泡框实例
    BubbleBox(title="重要提示", text="这是一条重要的消息，请注意查看！", color="#0088FF", height=50)
    BubbleBox(title="重要提示2", text="这是第二条消息，请注意查看！", color="#FF8800", height=100)
    BubbleBox(title="重要提示3", text="这是第三条消息，请注意查看！", color="#88FF00", height=150)
    BubbleBox.enqueue_bubble(title="hello", text="hello，请注意查看！", color="#8400FF", height=550)
    BubbleBox.enqueue_bubble(title="world", text="world，请注意查看！", color="#FF00F2", height=550)
    root.mainloop()
